...
bool waitForTCPConnection(void)
{
	...
	return WIFI_WaitServerConnection(socket, TCP_WAIT_TIMEOUT, remoteIP, remotePort) == WIFI_STATUS_OK;
}
...
{
	return WIFI_ReceiveData(socket, recData, REC_PAYLOAD_SIZE, &recDataLen) == WIFI_STATUS_OK;
}
